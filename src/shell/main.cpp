// src/shell/main.cpp
// Winix Shell — Patched (ANSI output, PATH injection, no duplicate Aliases)
...
#include "aliases.hpp"      // use the project's aliases class (avoid redefinition)
#include "completion.hpp"   // completion_matches(partial, aliases)
#include "line_editor.hpp"   // read_line_with_completion(...)
...
// -------------------------------
// Startup helpers: enable ANSI output and inject exe dir into PATH
// -------------------------------
static void enable_ansi_output_if_possible() {
    HANDLE hOut = GetStdHandle(STD_OUTPUT_HANDLE);
    if (hOut == INVALID_HANDLE_VALUE) return;
    DWORD mode = 0;
    if (!GetConsoleMode(hOut, &mode)) return;
    // enable virtual terminal processing for output only (colors, cursor control)
    mode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;
    SetConsoleMode(hOut, mode);
}

static void inject_exedir_into_path() {
    char buffer[MAX_PATH];
    if (!GetModuleFileNameA(NULL, buffer, (DWORD)sizeof(buffer))) return;
    std::string exePath = buffer;
    auto pos = exePath.find_last_of("\\/");
    if (pos == std::string::npos) return;
    std::string exeDir = exePath.substr(0, pos);

    // don't prefix repeatedly
    std::string cur = getenv_win("PATH");
    if (cur.rfind(exeDir + ";", 0) == 0) return; // already prefixed
    std::string updated = exeDir + ";" + cur;
    setenv_win("PATH", updated);
}
...
int main() {
    ...
    // Enable ANSI output (colors) — only output mode, avoid touching input mode
    enable_ansi_output_if_possible();

    // Ensure Winix-built commands (ls, cat, etc.) are found by PATH when launching
    inject_exedir_into_path();
