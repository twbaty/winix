// Winix Shell v1.11 — Native Logic, Env Handling, and Stable Background Execution
#include <windows.h>
#include <conio.h>
#include <io.h>
#include <cstdio>
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        ...
        if (bg) {
            ...
            BOOL ok = CreateProcessA(nullptr, buf.data(), nullptr, nullptr,
                FALSE, DETACHED_PROCESS | CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi);
...
static bool run_single(const std::vector<std::string>& argv) {
    ...
    // --- Expand envs before launch ---
    std::vector<std::string> expanded = argv;
    for (auto& a : expanded) a = expand_env_once(a);

    std::string cmdline = join_cmdline(expanded);
    std::vector<char> buf(cmdline.begin(), cmdline.end());
    buf.push_back('\0');
...
int main() {
    enable_vt_mode();
    std::cout << "Winix Shell v1.11 — Env + Background Stable\n";
